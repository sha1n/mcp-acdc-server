...
mcp-add-claude-dev: go-build-current
	...
	claude mcp add --transport stdio acdc-dev $(GOBIN)/$(PROGRAMNAME) -- --content-dir $(GOBASE)/examples/sample-content --uri-scheme sha1n --cross-ref
...
mcp-add-gemini-dev: go-build-current
	...
	gemini mcp add acdc-dev $(GOBIN)/$(PROGRAMNAME) -- --content-dir $(GOBASE)/examples/sample-content --uri-scheme sha1n --cross-ref
