...
-   **Duplicate identity**: If two discovered documents resolve to the same URI — for example, `guide.md` and `guide.markdown` selected side by side both produce `<scheme>://guide` — the first one the walk reaches is indexed and the second is skipped with a `WARN` naming both paths. Both discovery modes present files in sorted canonical-path order, so which document wins is stable across restarts.
